...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	 ...
	 * 未完成的FactoryBean实例的名称和BeanWrapper的映射缓存
	 */
	 ...
	 * 那么Spring会自动初始化B，这是Spring提供的一个重要特性。
	 * 但是，某些情况下，B不会被初始化，其中一种情况就是B实现了BeanNameAware接口。
	 * 自动装配时忽略给定的依赖接口，典型应用是通过其他方式解析Application上下文注册依赖，
	 * 类似于BeanFactory通过BeanFactoryAware进行注入
	 * 或者ApplicationContext通过ApplicationContextAware进行注入
	 */
	...
	protected Object createBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		// 判断需要创建的Bean是否可以实例化，是否可以通过类装载器载入
		// 锁定class，根据设置的class属性或者根据className来解析Class
		Class<?> resolvedClass = resolveBeanClass(mbd, beanName);
		if (resolvedClass != null && !mbd.hasBeanClass() && mbd.getBeanClassName() != null) {
			// 当bean类型可解析且bean定义的bean类型未解析且bean定义中的beanClass已定义(既为bean全类名)
		...
		try {
			...
			/*如果Bean配置了PostProcessor，那么这里返回的是bean的一个proxy*/
	...
	protected Object doCreateBean(final String beanName, final RootBeanDefinition mbd, final @Nullable Object[] args)
		...
		if (instanceWrapper == null) {
			// 根据指定bean使用对应的策略创建新的实例(工厂方法、构造函数自动注入、简单初始化)
		...
		if (earlySingletonExposure) {
			...
			if (earlySingletonReference != null) {
				...
				if (exposedObject == bean) {
					 ...
					 * actualDependentBeans不为空则表示当前bean创建后其所依赖的bean却没有没全部创建完，也就是说存在循环依赖
					 */
	 ...
	 * 在给定bean上调用指定的自定义init方法。叫invokeInitMethods。
	 * 可以在子类中覆盖初始化方法的自定义解析。
	 *
	 * @see #invokeInitMethods
	 */
