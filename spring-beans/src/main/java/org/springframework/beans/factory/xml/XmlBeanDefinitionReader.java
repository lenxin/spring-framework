...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	...
	public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
		...
		/*通过resourcesCurrentlyBeingLoaded的ThreadLocal对象，获取当前线程已经加载的资源文件*/
		Set<EncodedResource> currentResources = this.resourcesCurrentlyBeingLoaded.get();
		if (currentResources == null) {
			currentResources = new HashSet<>(4);
			this.resourcesCurrentlyBeingLoaded.set(currentResources);
		}
		/*将当前要加载的资源添加到已被加载的行列并判断资源是否已经加载，若已加载，抛出BeanDefinitionStoreException异常*/
	 ...
	 */
	/*获取XML文件的验证模式，常用DTD与XSD，DTD:文档类型定义(Document Type Definition)，XSD(XML Schemas Definition)*/
	...
	protected int detectValidationMode(Resource resource) {
		...
		try {
			/*将自动检测验证模式的工作委托给了专门处理类XmlValidationModeDetector*/
	...
	public int registerBeanDefinitions(Document doc, Resource resource) throws BeanDefinitionStoreException {
		...
		/*加载及注册Bean，应用了单一职责原则，将逻辑处理委托给单一BeanDefinitionDocumentReader类进行处理*/
